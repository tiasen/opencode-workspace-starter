...
验证命令约定（见 `AGENTS.md` §4.3）：

- 上表"验证命令"必须是**只覆盖本次改动面**的 scoped 命令（按改动文件/包运行）；仅当仓库确实只提供全量命令时才退化为全量。
- 若某仓的约定命令**基线为红**（存在与本次改动无关的既有失败），在此显式标注，禁止要求 Agent 调查或修复：`{repo}: {命令} — {既有失败说明}`。
...
- **是否需要跨仓审查**: 是 / 否
- **轻量通道**: 是 / 否（单仓、对外契约不变、改动面小时可为单 Writer + 跳过审查）
- **触发条件**: {外部可观察契约变更 / 依赖他仓规范 / 仓要求 / 用户要求；跳过则写"单仓内部、不改对外契约"}
