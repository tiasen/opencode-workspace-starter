...
1. 运行 `/prepare`（或读取 `openspec/repo-context.md`），确认各仓状态与 commit hash。
...
3. `tasks.md` 生成完毕**立即**用 Task 工具派发（`@frontend-writer`、`@backend-writer`……），无依赖并发、有依赖串行；Prompt 必须包含三件套：`context.md` + `design.md` + `specs/{target}.md`，外加本仓三要素（路径、Commit 短 hash、验证命令，取值以 `repo-context.md` 为准）。
...
- **派发必须 grounded in `/prepare` 结果**：派发前必须已读取 `openspec/repo-context.md`；文件缺失、未覆盖目标仓或 commit 已漂移时，先重跑 `/prepare` 再派发，禁止凭记忆或过期假设构造 Prompt。
